...
class CFuncWriter:
    ...
    def emit_stmt_AssignLocal(self, assign: ast.AssignLocal) -> None:
        ...
        if assign.value.w_T is not TYPES.w_NoneType:
    ...
    def _emit_propagate_err(
        ...
        suffix = self.ctx.result_suffix(self.w_func.w_functype.w_restype)
        fqn_str = str(self.fqn.human_name(self.ctx.vm)).replace('"', '\\"')
    ...
    def _fmt_raise_call(self, call: ast.Call) -> C.Expr:
        ...
        self._tmp_counter += 1
        self.tbc.wl(
            f"spy_Exc *SPY_exc_{n} = spy_exc_new({chain_var}, spy_StrObject_CHARS({c_msg}));"
        )
